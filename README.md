# groobee-developer-support (Groobee 개발자 지원 레포)

Groobee SDK 및 스크립트 사용하는 **개발자와 서비스 운영자**를 위한 지원 레포입니다.  
웹, iOS, Android 및 하이브리드 앱 환경에서 Groobee를 빠르고 안정적으로 연동하는 것을 목표로 합니다.

---

## 현재 최신 SDK 버전 (Stable)

| 플랫폼 | 최신 버전      | 변경 로그 |
| --- |------------| --- |
| Android SDK | **1.0.86** | [변경 로그 보기](docs/changelog/sdk-android-changelog.md) |
| iOS SDK | **1.1.12** | [변경 로그 보기](docs/changelog/sdk-ios-changelog.md) |

---

## 빠른 시작

- [Web 스크립트 설치 시작하기](docs/getting-started/gettingstart-web.md)
- [Android SDK 설치 시작하기](docs/getting-started/gettingstart-aos.md)
- [iOS SDK 설치 시작하기](docs/getting-started/gettingstart-ios.md)

---

## 연동 샘플

배포된 SDK/스크립트를 사용하는 **동작 가능한 예제**가 [`samples/`](samples/README.md) 에 있습니다.  
각 샘플의 README 에 키 설정·빌드·실행 방법이 정리되어 있습니다.

| 플랫폼 | 샘플 | 비고 |
| --- | --- | --- |
| Android (Kotlin) | [samples/app/android-kotlin](samples/app/android-kotlin) | Maven Central `groobee-sdk-message` |
| iOS (Swift) | [samples/app/ios-swift](samples/app/ios-swift) | CocoaPods `GroobeeKit` |
| Flutter | [samples/app/flutter](samples/app/flutter) | MethodChannel 브리지 (Android/iOS) |
| Web (일반) | [samples/web/vanilla](samples/web/vanilla) | 멀티 페이지, 자동 수집 + 직접 호출 |
| Web (React SPA) | [samples/web/react](samples/web/react) | `isSPA` 모드, 라우트 전환 추적 |

---

## 행동 유형 목록

행동 유형 목록과 행동 유형별 설명은    
👉 [행동 유형](docs/specs/action/README.md)를 참고해주세요.

---

## 트러블슈팅

자주 발생하는 이슈들 및 해결 방법은 [트러블슈팅 문서](docs/troubleshooting/README.md)에 정리되어 있습니다.  

👉 [트러블슈팅](docs/troubleshooting/README.md)

---

## 문의

문의사항이나 문제 제보는 GitHub Issues에 등록해주세요.  
재현 방법과 환경 정보를 함께 남겨주시면 빠르게 확인할 수 있습니다.  

👉 [Issues 바로가기](../../issues)
