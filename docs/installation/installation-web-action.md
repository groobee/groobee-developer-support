...
- 행동 유형별 상세한 설명은 👉 [행동 유형](../specs/action/README.md) 문서를 참고해주세요.
- 행동 유형에서 사용되는 데이터 스키마에 대한 설명은 👉 [Schema](../specs/action/schema.md) 문서를 참고해주세요.
  ...
  goods : [
    {
      ...
      prc: 25000,
      salePrc: 20000,
      status : "FS", // 품절이거나 상품이 판매상태가 아닐 경우 "SS"
      img: "상품 이미지 전체 URL",
      cat: "1234",
      cateNm: "티셔츠",
      catL: "1",
...
>   만약 상품 수 인풋 박스가 정상적으로 바뀌지 않는다면 추가된 Groobee 스크립트로 인해 문제가 발생한 것입니다.  
>   **&lt;input name=”amount”&gt;** 가 겹치면서 생기는 오류일 가능성이 높으므로. Groobee 장바구니 스크립트를 하단으로 이동시키면 됩니다.
...
### 장바구니 페이지 (VC)
- HTML 편집 > 페이지 편집 > 주문관련 > 장바구니로 이동합니다.
- 장바구니 에디터 하단의 페이지 코드 > 반복문 > {{$장바구니리스트}} 편집하기 > 리스트 반복 구문을 삽입합니다.  

[장바구니 편집 화면 위치](../images/installation/wisa/wisa-cart-pos.png)  
[장바구니 > 반복문 > 장바구니리스트 편집기 위치](../images/installation/wisa/wisa-cart-edt.png)  
[코드 삽입 예시](../images/installation/wisa/wisa-cart-exm.png) 

```html
<!-- Groobee Cart Selector Script -->
<div class="groobeeCartList" style="display: none;">
    <span class="groobeeProductA">{{$상품명(링크포함)}}</span> 
	<span class="groobeeProductName">{{$상품명}}</span>
	<span class="groobeeProductCount">{{$구매수량}}</span>
	<span class="groobeeProductAmount">{{$상품판매가격}}</span>
	<span class="groobeeProductPrice">{{$상품가격}}</span>
	<span class="groobeeProductSalePrice">{{$상품가격}}</span>
	<span class="groobeeProductImage">{{$상품이미지경로}}</span>
</div>
<!-- End of Groobee Cart Selector Script -->
```

### 주문서 작성 페이지 (OR)
- HTML 편집 > 페이지 편집 > 주문관련 > 주문서로 이동합니다.
- 주문서 에디터 하단의 페이지 코드 > 반복문 > {{$장바구니리스트}} 편집하기 > 리스트 반복 구문을 삽입합니다.  

[주문서 작성 편집 화면 위치](../images/installation/wisa/wisa-order-pos.png)  
[주문서 > 반복문 > 장바구니리스트 편집기 위치](../images/installation/wisa/wisa-order-edt.png)  
[코드 삽입 예시](../images/installation/wisa/wisa-order-exm.png)
...
<div class="groobeeOrderList" style="display: none;">
	...
	<span class="groobeeProductName">{{$상품명}}</span>
	<span class="groobeeProductCount">{{$구매수량}}</span>
	<span class="groobeeProductAmount">{{$상품총가격}}</span>
	<span class="groobeeProductPrice">{{$상품가격}}</span>
	<span class="groobeeProductSalePrice">{{$상품가격}}</span>
	<span class="groobeeProductImage">{{$상품이미지경로}}</span>
</div>
...
### 카테고리 페이지 (CA)
- 카테고리 스크립트는 상품 리스트 페이지에 추가하면 됩니다.
- HTML 편집 > 페이지 편집 > 상품관련 > 상품리스트로 이동해 아래 스크립트를 삽입합니다.  

[상품리스트 편집 화면 위치](../images/installation/wisa/wisa-category-pos.png)  
[코드 삽입 예시](../images/installation/wisa/wisa-category-exm.png)

```html
<!-- Groobee Selector Script -->
<div class="groobeeCategory" style="display:none">
    <span class="groobeeCategoryName">{{$카테고리명}}</span>
    <span class="groobeeCategoryCode">{{$카테고리코드}}</span>
</div>
<!-- End of Groobee Selector Script -->
```

### 상품 상세 페이지 (VG)
- 위사 쇼핑몰 관리자 페이지에 로그인 한 후, **디자인 > 디자인관리 > 스크립트 매니저**에 접속하여 상품상세 스크립트를 작성합니다.  
  [공통 스크립트 설치 가이드](../installation/installation-web-common-script.md)를 참고하여 스크립트 매니저에 스크립트를 추가한 경우,  
  해당 스크립트 내에 아래 예시의 상품 상세 스크립트 부분을 추가로 작성해주시면 됩니다.  

[스크립트 매니저 위치](../images/installation/wisa/wisa-script-mgr-pos-2.png)  
[상품 상세 스크립트 예시](../images/installation/wisa/wisa-detail.png)  
...
<div class="groobeeProductDetail" style="display: none;"> 
	...
	<span class="groobeeProductPrice">{상품가격}</span>
	<span class="groobeeProductSalePrice">{상품가격}</span>
	<span class="groobeeProductCode">{시스템코드}</span>
	<span class="groobeeProductImage">{상품이미지}</span>
	<span class="groobeeProductCategory"></span>
	<span class="groobeeProductCategoryName">{카테고리명}</span>
</div>
<!-- End of Groobee Selector Script -->
```

### 주문완료 페이지 (PU)
- 위사 쇼핑몰 관리자 페이지에 로그인 한 후, **디자인 > 디자인관리 > 스크립트 매니저**에 접속하여 주문완료 스크립트를 작성합니다.  
  [공통 스크립트 설치 가이드](../installation/installation-web-common-script.md)를 참고하여 스크립트 매니저에 스크립트를 추가한 경우,  
  해당 스크립트 내에 아래 예시의 주문완료 스크립트 부분을 추가로 작성해주시면 됩니다.


[스크립트 매니저 위치](../images/installation/wisa/wisa-script-mgr-pos-2.png)  
[주문완료 스크립트 예시](../images/installation/wisa/wisa-purchase.png)

```html
<!-- Groobee Selector Script -->
<meta property="groobee:order_no" content="{주문번호}">
<!-- End of Groobee Selector Script -->
```
...
- [추천 캠페인 사용법 가이드](installation-web-recommend.md)로 이동하여 추천 캠페인 사용법을 확인해주세요.
