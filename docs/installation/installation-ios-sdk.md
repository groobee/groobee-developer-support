...
| 클래스 | 메소드 | 설명 |
| --- | --- | --- |
| `GroobeeConfig` | `setServiceKey()` | 필수. Groobee 어드민에서 발급받은 서비스키를 등록합니다. |
...
Groobee의 세션과 메시지 처리를 위해 앱의 LifeCycle을 `GroobeeKitLifeCycle`에 연결합니다.
...
향후 확장될 메시지 유형들을 유연하게 제공하는 것을 목표로 하고 있으며, 사용자들에게 다양하고 원활한 정보 전달을 위해 Groobee는 Rich Push를 사용하고 있습니다.
