...
- Groobee 서비스키
- [앱 정보 등록](../prerequisites/app-name-registration.md)
...
1. Firebase 콘솔에서 프로젝트 개요를 열고 프로젝트 설정으로 진입합니다.
2. 프로젝트 설정에서 `클라우드 메시징` 탭을 엽니다.
3. `Apple 앱 구성`에서 사용 중인 iOS 앱의 Bundle ID를 확인하고 `APN 인증 키` 업로드를 진행합니다.
4. `.p8` 파일, Key ID, Team ID를 입력해 업로드합니다.
...
1. `+ Capabilities`에서 `Push Notifications`, `Background Modes`를 추가하고 활성화합니다.
2. `TARGETS` 하단의 `+` 버튼을 클릭합니다.
3. `Notification Service Extension`을 선택해 추가합니다.
4. 스키마를 활성화합니다.
5. Service의 Deployment Info에서 iOS 버전을 현재 앱의 Deployment Target과 동일하게 맞추고, `GroobeeKit.xcframework`를 추가한 뒤 `Embed`를 `Do Not Embed`로 설정합니다.
...
2. `Notification Content Extension`을 선택해 추가합니다.
3. 스키마를 활성화합니다.
4. Content의 Deployment Info에서 iOS 버전을 현재 앱의 Deployment Target과 동일하게 맞추고, `GroobeeKit.xcframework`를 추가한 뒤 `Embed`를 `Do Not Embed`로 설정합니다.
