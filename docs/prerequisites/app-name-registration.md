...
Groobee는 이를 웹뷰의 **User-Agent 문자열**로 구분합니다. 앱에서 웹뷰에 커스텀 User-Agent 값을 지정한 뒤, Groobee 어드민의 `사용 플랫폼` 항목에도 같은 값을 등록해두면, 해당 User-Agent로 들어오는 요청이 "앱 내부 웹뷰 트래픽"으로 식별됩니다.
