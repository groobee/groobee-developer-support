...
> **본 내용은 현재 최신 권장 SDK 버전 기준**의 기능 지원 상태입니다. 특정 이전 버전에서의 지원 여부나 버전별 변경 내역은 [iOS SDK 변경 로그](./sdk-ios-changelog.md)에서 확인하세요.
...
| 서비스키 등록 | `GroobeeConfigBuilder.setServiceKey` | ✅ 지원 | 필수, `serviceKey`와 `bundleId` 전달 |
