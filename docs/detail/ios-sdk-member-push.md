...
Groobee 메소드는 가급적이면 View 생성 시 한 번만 호출되는 `viewDidLoad()` 같은 곳에서 사용하시길 권합니다.  
...
- Groobee에 등록된 회원 ID에 한해서 타겟팅할 수 있게 되며, 그 대상으로 메시지를 전달할 수 있습니다.
...
- Groobee는 이 동의 여부를 기준으로 발송 대상을 만들어 푸시 메시지를 전송합니다.
