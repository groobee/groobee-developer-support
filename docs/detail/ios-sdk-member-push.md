...
| 변수명 | 자료형 | 설명 | 예시 |
| --- | --- | --- | --- |
| `USER_ID_STRING` | `String` | 회원아이디 | `test123@test.com` or `test123` |

주의:

- 로그인 시에만 `setServiceLogin()`을 호출하면 기존에 이미 로그인되어 있던 사용자가 비회원 정보로 쌓일 수 있습니다.
...
| 변수명 | 자료형 | 설명 | 예시 |
| --- | --- | --- | --- |
| `id` | `String` | 회원아이디 | `test123@test.com` or `test123` |
...
| 변수명 | 자료형 | 설명 |
| --- | --- | --- |
| `userInfo` | `[String: Any]` | 필수 요청값: `id`, `grade`, `age`, `gender`, `type` / 추가값: `vendorCd`, `point` |
...
| 변수명 | 자료형 | 설명 | 예시 |
| --- | --- | --- | --- |
| `USER_JOIN_ID` | `String` | 회원가입 계정 | `test123@test.com` or `test123` |
...
| 변수명 | 자료형 | 설명 | 예시 |
| --- | --- | --- | --- |
| `MEMBER_ID` | `String` | 회원아이디 | `test123@test.com` or `test123` |
...
- 유저가 설정에서 푸시 사용에 대한 클릭이 발생할 때만 호출할 경우 초기값이 설정되지 않아 허용 여부가 `false`로 들어갈 수 있습니다.
- 이를 고려해 적절한 위치에서 `setPushAgreeAP()` 메소드를 한 번 더 호출해야 합니다.

설명:

- 그루비는 푸시 사용 동의 여부를 받아 푸시 동의를 수락한 유저에 한해 발송대상을 만들어 푸시 메시지를 전송합니다.
...
- 유저가 설정에서 푸시 사용에 대한 클릭이 발생할 때만 호출할 경우 초기값이 설정되지 않아 허용 여부가 `false`로 들어갈 수 있습니다.
- 이를 고려해 적절한 위치에서 `setPushAgreeAA()` 메소드를 한 번 더 호출해야 합니다.

설명:

- 그루비는 광고 푸시 사용동의 여부를 받아 동의를 수락한 유저에 한해 발송대상을 만들어 광고 메시지를 발송할 수 있습니다.
...
- 유저가 설정에서 푸시 사용에 대한 클릭이 발생할 때만 호출할 경우 초기값이 설정되지 않아 허용 여부가 `false`로 들어갈 수 있습니다.
- 이를 고려해 적절한 위치에서 `setPushAgreeAN()` 메소드를 한 번 더 호출해야 합니다.
